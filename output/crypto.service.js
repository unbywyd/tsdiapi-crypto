...
let CryptoService = class CryptoService {
     ...
     */
    generateMasterKeyFromComponents(components, hashAlgorithm = 'sha256') {
    ...
    async deriveEncryptionKey(password, salt) {
        ...
        return await subtle.importKey('raw', derivedKey, { name: 'AES-GCM' }, false, // Не экспортируемый ключ
        ['encrypt', 'decrypt']);
    }
    // ================================
    // ========== KEY GENERATION ======
     ...
     */
    async generateSigningKeyPair(format = 'cryptoKey') {
     ...
     * @returns Публичный и приватный ключи в указанном формате.
     */
    async generateEncryptionKeyPair(format = 'cryptoKey') {
        const keyPair = await subtle.generateKey({
            name: 'RSA-OAEP',
            modulusLength: 2048,
            publicExponent: new Uint8Array([1, 0, 1]),
     ...
     */
    async generateSymmetricKey(format = 'cryptoKey') {
     ...
     * @returns PEM строка ключа.
     */
    async convertCryptoKeyToPem(key, type) {
     ...
     */
    async convertPemToCryptoKey(pem, type, algorithm, usages) {
        const arrayBuffer = this.pemToArrayBuffer(pem);
        const format = type === 'public' ? 'spki' : 'pkcs8';
        return await subtle.importKey(format, arrayBuffer, algorithm, true, usages);
    }
    /**
     ...
     */
    async convertCryptoKeyToBase64(key, format) {
     ...
     * @returns CryptoKey объект.
     */
    async convertBase64ToCryptoKey(base64Key, format, algorithm, usages) {
     ...
     */
    async encryptStringWithMasterKey(data) {
        ...
        const encryptedData = await subtle.encrypt({ name: 'AES-GCM', iv }, derivedKey, dataArray);
    ...
    /**
         ...
         */
    async encryptKeyWithMasterKey(key, keyType) {
        let exportFormat;
        ...
        const encryptedData = await subtle.encrypt({ name: 'AES-GCM', iv }, derivedKey, keyBytes);
   ...
   */
    async decryptSymmetricKeyWithPrivateKey(encryptedSymmetricKey, privateKey) {
        ...
        return await subtle.importKey('raw', decryptedKeyBuffer, { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
    }
    /**
     ...
     */
    async decryptStringWithMasterKey(encryptedData) {
        ...
        const decryptedData = await subtle.decrypt({ name: 'AES-GCM', iv }, derivedKey, ciphertext);
     ...
     */
    async decryptKeyWithMasterKey(encryptedKey, keyPurpose) {
        ...
        const decryptedData = await subtle.decrypt({ name: 'AES-GCM', iv }, derivedKey, encryptedData);
        ...
        return await subtle.importKey(importFormat, decryptedData, algorithm, true, usages);
    }
    /**
     ...
     */
    async signData(data, privateKey, outputFormat = 'ArrayBuffer') {
    ...
    async verifySignature(data, signature, publicKey) {
        ...
        return await subtle.verify({
            name: 'RSA-PSS',
            saltLength: 32,
        }, publicKey, signatureArray, dataArray);
    }
    // ================================
    ...
    */
    async encryptSymmetricKeyWithPublicKey(symmetricKey, publicKey) {
     ...
     */
    async encryptSymmetricKeyWithSymmetricKey(keyToEncrypt, encryptingKey) {
        const rawKey = await subtle.exportKey('raw', keyToEncrypt);
        const keyBytes = new Uint8Array(rawKey);
        const iv = crypto.randomBytes(12); // 96-bit IV for AES-GCM
        const encryptedData = await subtle.encrypt({ name: 'AES-GCM', iv }, encryptingKey, keyBytes);
     ...
     */
    async decryptSymmetricKeyWithSymmetricKey(encryptedKey, decryptingKey) {
        const combined = Buffer.from(encryptedKey, 'base64');
        const iv = combined.slice(0, 12); // First 12 bytes
        const encryptedData = combined.slice(12); // Rest is encrypted key
        const decryptedData = await subtle.decrypt({ name: 'AES-GCM', iv }, decryptingKey, encryptedData);
        return await subtle.importKey('raw', decryptedData, { name: 'AES-GCM' }, true, ['encrypt', 'decrypt']);
    }
    // ================================
     ...
     */
    async encryptPrivateKeyWithSymmetricKey(privateKey, symmetricKey, keyPurpose) {
        const keyData = await subtle.exportKey('pkcs8', privateKey);
        const keyBytes = new Uint8Array(keyData);
        const iv = crypto.randomBytes(12); // 96-bit IV for AES-GCM
        const encryptedData = await subtle.encrypt({ name: 'AES-GCM', iv }, symmetricKey, keyBytes);
        // Combine IV + encryptedData
        const combined = Buffer.concat([iv, Buffer.from(encryptedData)]);
        return combined.toString('base64');
    }
     ...
     */
    async decryptPrivateKeyWithSymmetricKey(encryptedPrivateKey, symmetricKey, keyPurpose) {
        const combined = Buffer.from(encryptedPrivateKey, 'base64');
        const iv = combined.slice(0, 12); // First 12 bytes
        const encryptedData = combined.slice(12); // Rest is encrypted key
        const decryptedData = await subtle.decrypt({ name: 'AES-GCM', iv }, symmetricKey, encryptedData);
        ...
        return await subtle.importKey('pkcs8', decryptedData, algorithm, true, usages);
    }
    // ================================
    // ======== MASTER KEY METHODS =====
    // ================================
     ...
     */
    async encryptDataWithSymmetricKey(data, symmetricKey) {
        // Если данные представлены в виде строки, конвертируем их в Uint8Array
        const dataArray = typeof data === 'string' ? new TextEncoder().encode(data) : data;
        const iv = crypto.randomBytes(12); // 96-bit IV for AES-GCM
        const encryptedData = await subtle.encrypt({ name: 'AES-GCM', iv }, symmetricKey, dataArray);
        // Combine IV + encryptedData
        const combined = Buffer.concat([iv, Buffer.from(encryptedData)]);
        return combined.toString('base64');
    }
     ...
     */
    async decryptDataWithSymmetricKey(encryptedData, symmetricKey, outputFormat = 'string') {
        const combined = Buffer.from(encryptedData, 'base64');
        const iv = combined.slice(0, 12); // Первые 12 байт - это IV
        const data = combined.slice(12); // Остальные байты - зашифрованные данные
        const decryptedData = await subtle.decrypt({ name: 'AES-GCM', iv }, symmetricKey, data);
